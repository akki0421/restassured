...
public class SlotFinding {
    static String startDate;
    static String endDate;
    static JsonArray ListOfAvailableSlotsOnTheGivenDate;

    public static Object[] slotFinding() throws IOException {
        String date = CurrentDate.currentDate();
        Cookies cookie = given().body(new File("src/test/java/Files/PayLoad/login.json")).when().post("https://supply-backend-test.spinny.com/api/login/").then().extract().response().getDetailedCookies();
        ...
        DocumentContext json = JsonPath.using(configuration).parse(new File("src/test/java/Files/PayLoad/slotfindingRequest.json"));
